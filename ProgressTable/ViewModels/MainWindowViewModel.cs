...
namespace ProgressTable.ViewModels
{
    public class MainWindowViewModel : ViewModelBase
    {
        ...
        private void CheckAverage(Student[] students)
        {
            ...
            ScoreFizraSr /= students.Length;
            ColorFizraSr = checkColor(ScoreFizraSr);
        ...
        public MainWindowViewModel()
        {
            AddStudent = ReactiveCommand.Create(() =>
            {
                if (newName != "")
                {
                    ...
                    temp[temp.Length - 1] = new Student { Name = newName, Visual = scores[0], Architecture = scores[1], Networks = scores[2], Calculate_Math = scores[3], Fizra = scores[4], Math = scores[5], Electric = scores[6] };
                    Students = temp;
                    NewName = "";
                    ScoreVisual = 0;
                    ScoreArchitecture = 0;
            ...
            {
                new Student{Name="Гусев Лукьян Матвеевич", Visual=2, Architecture=1, Networks=1, Calculate_Math=2, Fizra=0, Math=1, Electric=1},
                new Student{Name="Сергеев Владислав Иванович", Visual=1, Architecture=1, Networks=0, Calculate_Math=2, Fizra=1, Math=2, Electric=0},
                new Student{Name="Смирнов Осип Федорович", Visual=0, Architecture=2, Networks=0, Calculate_Math=1, Fizra=2, Math=0, Electric=1},
                new Student{Name="Бобылёв Станислав Иосифович", Visual=1, Architecture=2, Networks=2, Calculate_Math=1, Fizra=2, Math=1, Electric=0},
            };
            CheckAverage(students);
        ...
        public ushort ScoreFizra { get => scores[4]; set => this.RaiseAndSetIfChanged(ref scores[4], value); }
        ...
        public float ScoreFizraSr { get => sc_scores[4]; set => this.RaiseAndSetIfChanged(ref sc_scores[4], value); }
        ...
        public SolidColorBrush ColorFizraSr { get => colorBrush[4]; set => this.RaiseAndSetIfChanged(ref colorBrush[4], value); }
